...
- This is a StencilJS design system monorepo distributed by CDN from Netlify, not an npm-published package for now (`private: true`).
- The root uses npm workspaces: `apps/docs` for the published site and aggregate bundle, `packages/*/*` for independently versioned component packages and shared tokens grouped by Atomic Design level.
- Netlify publishes root `www`; `/` is the Stencil demo index, `/demos/*.html` are per-component HTML demos, `/storybook/` is Storybook, `/build/iv-basic-ds.esm.js` is the aggregate CDN ESM bundle, and `/components/<component>/<version>/build/<component>.esm.js` is the versioned component CDN bundle.
...
- Use `npm run deploy:netlify` as the main verification command before deploy; it runs workspace Stencil Vitest specs with coverage, builds Netlify output, copies docs coverage to `www/coverage`, installs the Playwright Chromium browser when needed, then runs Playwright + axe-core with the HTML report in `www/test-report`.
...
- `apps/docs/.storybook/preview.ts` imports `../www/build/iv-basic-ds.esm.js`, so Storybook depends on `apps/docs/www/build` existing.
...
- `apps/docs/stencil.config.ts` uses namespace `iv-basic-ds`; changing it changes aggregate CDN filenames referenced by `apps/docs/src/index.html`, README, and Storybook.
- Each `packages/<atomic-level>/<component>/stencil.config.ts` uses that component namespace; changing it changes the versioned CDN filenames.
- Output targets are `www`, `dist`, and `dist-custom-elements`; root `www/build` is the aggregate runtime CDN output and root `www/components/<component>/<version>/build` is the versioned component output.
...
- The aggregate docs app follows Atomic Design physically under `apps/docs/src/components/<atomic-level>/<component>/`, where `<atomic-level>` is `atoms`, `molecules`, `organisms`, or `templates`.
- Versioned component package source lives under `packages/<atomic-level>/<component>/src/components/<component>/`.
...
- Put versioned component files under `packages/<atomic-level>/<component>/src/components/<component>/` with the Stencil file, CSS file, and spec next to each other. Keep docs stories in `apps/docs/src/components/<atomic-level>/<component>/`.
- Keep CSS class names prefixed with `iv-` because there is no Shadow DOM encapsulation.
- Use shared tokens from `packages/foundations/tokens/src/tokens.css`; token names should use the `--iv-` prefix. The docs app also keeps `apps/docs/src/global/tokens.css` for the aggregate bundle.
...
- Add or update a Storybook story for component states in `apps/docs/src`. Put HTML demos in `apps/docs/src/demos/<component>.html` and link them from `apps/docs/src/index.html`; avoid growing `apps/docs/src/index.html` into a full demo page.
